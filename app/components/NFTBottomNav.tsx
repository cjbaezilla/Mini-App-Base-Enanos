  ...
  return (
    <nav className="bg-[var(--app-paper-bg)] border-t-2 border-[var(--app-accent)] shadow-[var(--app-shadow-nav)] relative">
      <div className="flex items-center justify-between py-2 px-4 relative">
          ...
          {navItems.filter(item => item.id === 'home').map((item) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center space-y-1 py-1 px-3 rounded-lg transition-all duration-200 ${
                  isActive
        ...
        <div className="flex justify-center">
          {navItems.find(item => item.id === 'mint') && (
            <button
              ...
              className={`flex flex-col items-center space-y-1 py-3 px-4 rounded-2xl absolute z-20 -top-6 left-1/2 -translate-x-1/2 transition-all duration-200 ${
                activeTab === 'mint'
        ...
        {/* Botón derecho (Enanos) */}
        <div className="flex-1 flex justify-center">
          {navItems.filter(item => item.id === 'nft').map((item) => {
            const isActive = activeTab === item.id;
